...
class MultiTouchListener implements OnTouchListener {
    ...
    private com.reactlibrary.photoedit.OnPhotoEditorSDKListener onPhotoEditorSDKListener;

    MultiTouchListener(View deleteView, RelativeLayout parentView,
                       ImageView photoEditImageView, com.reactlibrary.photoedit.OnPhotoEditorSDKListener onPhotoEditorSDKListener) {
    ...
    private void firePhotoEditorSDKListener(View view, boolean isStart) {
        if (view instanceof TextView) {
            if (onMultiTouchListener != null) {
                if (onPhotoEditorSDKListener != null) {
                    if (isStart)
                        onPhotoEditorSDKListener.onStartViewChangeListener(com.reactlibrary.photoedit.ViewType.TEXT);
                    else
                        onPhotoEditorSDKListener.onStopViewChangeListener(com.reactlibrary.photoedit.ViewType.TEXT);
                }
            } else {
                if (onPhotoEditorSDKListener != null) {
                    if (isStart)
                        onPhotoEditorSDKListener.onStartViewChangeListener(com.reactlibrary.photoedit.ViewType.EMOJI);
                    else
                        onPhotoEditorSDKListener.onStopViewChangeListener(com.reactlibrary.photoedit.ViewType.EMOJI);
                }
            }
        } else {
            if (onPhotoEditorSDKListener != null) {
                if (isStart)
                    onPhotoEditorSDKListener.onStartViewChangeListener(com.reactlibrary.photoedit.ViewType.IMAGE);
                else
                    onPhotoEditorSDKListener.onStopViewChangeListener(com.reactlibrary.photoedit.ViewType.IMAGE);
            }
        }
    }
    ...
    private class ScaleGestureListener extends ScaleGestureDetector.SimpleOnScaleGestureListener {
        ...
        private com.reactlibrary.photoedit.Vector2D mPrevSpanVector = new com.reactlibrary.photoedit.Vector2D();
        ...
        public boolean onScale(View view, ScaleGestureDetector detector) {
            ...
            info.deltaAngle = isRotateEnabled ? com.reactlibrary.photoedit.Vector2D.getAngle(mPrevSpanVector, detector.getCurrentSpanVector()) : 0.0f;
